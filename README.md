...
> twenty-nine privilege-denial checks in CI.
...
`uv lock --check` · 25 SQL smoke tests on a clean rebuild · 915 Python tests
...
| 915 Python tests, 905 of them in one run against live Postgres | fde-mcp 142 · fde-agents 117 · fde-training 183 · fde-gate 303 · fde-sor 170. The 10 that skip need the `train` extra's heavy deps (9) or `wal_level=logical` (1); CI installs the extra and re-runs 39 of the fde-training tests in a job of its own |
