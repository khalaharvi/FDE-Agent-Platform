...
**What you are marketing.** Three forward-deployed-engineering agents on Amazon Bedrock AgentCore over a Postgres+pgvector knowledge graph, with deterministic human gates. The single most marketable idea is the invariant: *agents propose, humans dispose* — enforced in SQL at four layers and asserted by twenty-nine CI privilege-denial checks. Secondary hooks: every human review decision becomes training data (the flywheel), a no-AWS-needed quick start, an explicit cost dial (model presets), and a rare "Status and honesty" culture — seventeen real bugs candidly logged in `docs/99-sources.md` §7–§8, plus an eighteenth recorded open rather than quietly carried. Ground every claim you make in `README.md` and `docs/` before writing it.
