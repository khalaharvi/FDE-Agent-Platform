"""fde_mcp.tools -- the 18 MCP tool implementations, grouped by concern.
...
into their own modules (rather than one 1,400-line file) means a change to
...
from fde_mcp.tools import drift, graph, proposals, workflow
...
def register_all(mcp: FastMCP[None]) -> None:
    """Register all 18 tools (9 graph, 3 proposal, 3 drift, 3 workflow)."""
    graph.register(mcp)
