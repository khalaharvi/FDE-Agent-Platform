...
async def dashboard_page(request: Request) -> Response:
    ...
    The panels arrive as pre-rendered HTML rather than as rows -- see the
    `Markup` note below, which is the one place in this console autoescaping
    ...
    return await _page(
        request,
        "dashboard.html.j2",
        # THE SINGLE DELIBERATE AUTOESCAPE EXCEPTION IN THIS CONSOLE.
        #
        # Every other template renders untrusted values through autoescaping
        # or `|tojson` (see this module's docstring), and that stays true:
        # marking the string here rather than writing `|safe` in the template
        # keeps the exception to one auditable Python line instead of a piece
        # of template syntax the next page can copy without the argument.
        #
        # Why it is sound. This markup was generated by this process a moment
        # ago by `fde_mcp.dashboard`, which escapes every data-derived value
        # it interpolates -- reviewer principals, gate kinds, statuses,
        # commit digests, the engagement id -- with
        # `html.escape(..., quote=True)`, and three tests fail if that
        # escaping is removed. Nothing from the request reaches the string
        # unescaped; `engagement_id` and `window_days` are read back through
        # the escaper like any other datum.
        #
        # Why not render the rows in Jinja instead, which would need no
        # exception at all: the console, `/api/dashboard.md` and the MCP tool
        # must agree on every number, and a second hand-written copy of this
        # layout in a template is exactly the divergence `fde_mcp.dashboard`
        # exists to prevent. One renderer, three surfaces, one escape hatch.
        dashboard_html=Markup(built["html"]),  # noqa: S704 -- see above
