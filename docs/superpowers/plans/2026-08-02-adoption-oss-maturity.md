...
- **Branching:** each task gets its own worktree and branch off `origin/main` (`git worktree add <repo-root>/.claude/worktrees/<name> origin/main -b <branch>` from the repo root `/Users/krishnahalaharvi/Downloads/fde-platform-2`). One PR per task, `gh pr create --base main`. The user merges; never merge or push to main directly. NEVER touch the main checkout's working tree (`/Users/krishnahalaharvi/Downloads/fde-platform-2`) — it has in-flight work on `feat/one-click-deploy`; the sole exception is Task 2 moving the untracked `docs-site/` directory out of it, and Task 5 reading/committing the untracked `.claude/agents/marketing-strategist.md`.
...
1. Move the untracked `docs-site/` directory from `/Users/krishnahalaharvi/Downloads/fde-platform-2/docs-site` into this task's worktree root (`git mv` does not apply — it is untracked; plain `mv`). 12 pages + `docs.json` expected; run `mint validate` to confirm the import is intact before changing anything.
...
1. Commit the existing `.claude/agents/marketing-strategist.md` (read it from `/Users/krishnahalaharvi/Downloads/fde-platform-2/.claude/agents/marketing-strategist.md`) so the review loop ships with the repo.
