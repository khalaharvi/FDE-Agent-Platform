...
A third pass — the product-ops usability work that added the playbook renderer,
the reviewer admin, transcript intake, the agent launcher and the operator
plugin — found four more, all of them older than the work that surfaced them.
Three are fixed. The remaining one is recorded open rather than quietly fixed,
because closing it changes a model-facing tool signature and that is not a
thing to slip into a feature branch.

| Finding | Was | Now |
|---|---|---|
| `StepSpec.branches` disagrees with the runner about its own shape | `wf_draft` types `branches` as `dict[str, Any]` (`packages/fde-mcp/src/fde_mcp/tools/workflow.py:56`, mirrored at `packages/fde-agents/src/fde_agents/common/models.py:233`), while `DecisionExecutor` evaluates a *list* of `{"when","goto"}` objects with an optional `{"else"}` (`packages/fde-gate/src/fde_gate/executors.py:453`). No decision step an agent can author is one the runner can execute -- the two ends have never agreed, and nothing authored a decision step end to end to notice | **open.** The column is `jsonb`, so the gate's `make_workflow` fixture writes the list shape with raw SQL and the playbook surfaces are tested against what the runner actually executes, not the fallback. The fix is a signature change to a model-facing tool and is deliberately not folded into a docs pass |
| The run detail page answered "not found" for every run | `wf.run` has an `error` column, so `runs.get_run` returns a dict carrying an `error` key for every run that exists -- usually null. Both callers decided not-found with `"error" in result`, true for all of them: `GET /ui/runs/{id}` rendered "There is no run N here" and `GET /api/runs/{id}` answered 404 for runs sitting in the table. The column and the not-found envelope had shared a name since they were written, and nothing exercised either handler | `runs.is_missing` tests for the presence of `run_id`, which is what actually separates the two shapes; both callers use it |
| Nine of the twenty-five CI denials could not detect the break they assert | `assert_denied` reports `ok` for *any* failure, so a statement that dies before a privilege is ever consulted keeps passing after the grant it guards has widened. Proven by granting the privilege and watching the line fail anyway. Three were among the original eight, untouched since they were written: the `kg.node` INSERT named `commit_id 1`, which no database has ever had -- `db/rebuild.sh`'s smoke test consumes the identity values and rolls back, so even the demo seed's commits start at 7 -- and the two `SECURITY DEFINER` calls pass a proposal and a workflow id that do not exist, so the body raises "not found" even for a caller that holds `EXECUTE`. Six more were in db/016's roster block: two INSERTs named `reviewer_id 1`; the reviewer DELETE and the `principal` / `gate_kind` UPDATEs hit a foreign key or a unique constraint against any populated roster; and the two identical reviewer INSERTs are asserted for two roles, so the first one breaking masks the second | every write in the matrix now matches no rows -- `WHERE false`, or a literal source on the INSERTs -- which Postgres still plans and therefore still permission-checks, so the missing grant is the only thing left to fail on. The standard is now both directions: denied for the role under test, **and** succeeding once the privilege is granted, which is what makes a widened grant detectable. The count is unchanged at twenty-five |
| A run parked on a human step returned 500 | `wf.begin_step` stores the run context as the attempt's input (`db/013:585-586`) and `wf.start_run` seeds that context as `{"input": ...}` (`db/013:540`), so `s.input` is a non-empty object on every parked step of every run -- even one started with no input. `run.html.j2`'s `s.input.escalated_error` therefore raised `UndefinedError` under `StrictUndefined` instead of reading false. Every earlier test left `run.awaiting` empty, which is how the template's one interesting branch went unrendered for so long | reads through `.get`; the regression test starts a run with no input, advances it until it parks on a human step, and asserts both routes |
